...
def scrape_reuters(file_name="reuters_articles.csv"):
	...
	for article in articles:
		...
		if article.findAll("span",{"class": "timestamp"}):
			...
			if "ago" in timestamp:
				if "m ago" in timestamp:
					timestamp = str(datetime.now()-timedelta(minutes=int(timestamp.replace("m ago", ""))))
				elif "h ago" in timestamp:
					timestamp = str(datetime.now()-timedelta(hours=int(timestamp.replace("h ago", ""))))
